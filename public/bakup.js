...
var argv = minimist(process.argv.slice(2), {
	default: {
		...
		ws_uri: "ws://40.68.138.182:8888/kurento",
	},
});
...
function startScreenSharing(socket, roomname) {
	socket.joinForScreenSharing = true;
	socket.join(roomname, () => {
		getRoom(socket, roomname, (err, myRoom) => {
			if (err) {
				console.error(err);
			...
			myRoom.pipeline.create("WebRtcEndpoint", (err, outgoingMedia) => {
				if (err) {
					console.error("Error creating outgoingMedia:", err);
					return;
				}

				const user = {
					id: socket.id,
					name: "ScreenShare", // You can set a custom name for the screen-sharing user.
					outgoingMedia: outgoingMedia,
					incomingMedia: {},
				};

				// Add the user to the room for screen sharing
				myRoom.participants[user.id] = user;
				...
				// Notify the client that screen sharing started
				socket.emit("message", {
					event: "startScreenSharing",
					userid: user.id,
				});
			});
		});
